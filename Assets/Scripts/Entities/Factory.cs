...
public sealed class Factory : BaseEntity
{
    ...
    [SerializeField] int MaxBuildingQueueSize = 5;
    ...
    {
        get { return CurrentState == State.UnderConstruction; }
    }

    public int Cost
    {
        get { return FactoryData.Cost; }
    }

    public FactoryDataScriptable GetFactoryData
    {
        get { return FactoryData; }
    }

    public int AvailableUnitsCount
    {
        get { return Mathf.Min(MaxAvailableUnits, FactoryData.AvailableUnits.Length); }
    }

    public int AvailableFactoriesCount
    {
        get { return Mathf.Min(MaxAvailableFactories, FactoryData.AvailableFactories.Length); }
    }
    ...
    {
        get { return CurrentState == State.BuildingUnit; }
    }
    ...
    public override void UpdateVisibility(bool isVisible)
    {
        ...
        for (int i = 0; i < max; i++)
        {
            ...
            BuildGaugeImage.enabled = isVisible;
        }
    }
    ...
    public int GetMostExpensiveAffordableUnitIndex(int cost)
    {
        int len = UnitPrefabs.Length;
        int max = -1;

        for (int i = 0; i < len; i++)
        {
            bool affordable = UnitPrefabs[i].GetComponent<Unit>().Cost <= cost;
    ...
    public int GetMostExpensiveAffordableFactoryIndex(int cost)
    {
        int len = FactoryPrefabs.Length;
        int max = -1;

        for (int i = 0; i < len; i++)
        {
            bool affordable = FactoryPrefabs[i].GetComponent<Factory>().Cost <= cost;
    ...
    public GameObject GetFactoryGO(int factoryIndex)
    {
        return FactoryPrefabs[factoryIndex];
    }

    #endregion

    public void BuildUnitByLabel(string unitDataCaption)
    {
        for (int i = 0; i < UnitPrefabs.Length; i++)
        {
            if (UnitPrefabs[i].GetComponent<Unit>().GetUnitData.Caption == unitDataCaption)
            {
                RequestUnitBuild(i);
                return;
            }
