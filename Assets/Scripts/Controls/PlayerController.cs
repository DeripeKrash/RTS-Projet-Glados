...
public sealed class PlayerController : UnitController
{
    ...
    override protected void Start()
    {
        ...
        {
            Unit[] unitsToBeDestroyed = squadController.selectedUnits.ToArray();
    ...
    void StartSelection()
    {
        ...
        PlayerMenuController?.SquadMenuRaycaster.Raycast(MenuPointerEventData, results);
    ...
    void ComputeUnitsAction()
    {
        ...
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, targetMask))
        {
            ...
            if (target != null && target.GetTeam() != GetTeam())
            {
                ...
                squadController.Capture(target);
            }
        }
        ...
        else if (Physics.Raycast(ray, out raycastInfo, Mathf.Infinity, floorMask))
        {
            ...
            SetTargetCursorPosition(raycastInfo.point);

            // Direct call to moving task $$$ to be improved by AI behaviour
            if (Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.LeftShift))
            {
                squadController.OffensivelyMoveTo(raycastInfo.point);
            }
            else
            {
                squadController.MoveTo(raycastInfo.point);
            }
        }
    }
    #endregion
